...
router.get("/", tokenVerify, async (req, res) => {
  ...
  for (const item of query2) {
    ...
    itemData[index] = {
      url: query3[0].image_url,
      unitPrice: query3[0].price,
      totalPrice: itemPrice,
      name: query3[0].product_name,
  ...
  async (req, res) => {
    ...
    if (info.start === 1) {
      ...
      const [rows] = await pool
        .promise()
        .query(sql_2, [req.cookies.order_id, info.product_id, info.num]);
...
router.post(
  ...
  async (req, res) => {
    ...
    const sql =
      "UPDATE orders SET status = ?, instructions = ?, table_no = ? WHERE order_id = ?";
    const query = await pool
      .promise()
      .query(sql, [
        "Yet to start",
        req.body.message,
        req.body.table,
        req.cookies.order_id,
      ]);

    const user = jwt.verify(req.cookies.token, SECRET_KEY);
    const sql2 =
      "INSERT INTO payments (order_id, user_id, created_at, food_total, tip) VALUES (?, ?, NOW(), ?, ?);";
    const query2 = await pool
      .promise()
      .query(sql2, [req.cookies.order_id, user.user_id, price, req.body.tip]);

    //cleanup
    const sql3 =
      "DELETE FROM payments p WHERE NOT EXISTS (SELECT 1 FROM serve s WHERE s.order_id = p.order_id);";
    const query3 = await pool.promise().query(sql3);
    const sql5 =
      "DELETE FROM orders o WHERE NOT EXISTS (SELECT 1 FROM serve s WHERE s.order_id = o.order_id);";
